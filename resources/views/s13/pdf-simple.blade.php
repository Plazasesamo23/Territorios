        ...
        <table>
            ...
            <tbody>
                @foreach($territoriosPagina as $territorio)
                    @php
                        $ultimaFecha = null;
                        $registros = $territorio->registros ?? collect();
                        if($registros->count() > 0) {
                            // CORREGIDO: Solo mostrar fecha si el registro MÁS RECIENTE está completado
                            $ultimoRegistro = $registros->sortByDesc('fecha_salida')->first();
                    ...
                    <tr>
                        ...
                        <td colspan="2" class="sub-col">
                            @if(isset($registros[0]) && $registros[0]->publicador)
                                {{ $registros[0]->publicador->nombre }} {{ $registros[0]->publicador->apellidos }}
                            @endif
                        </td>

                        <!-- Asignación 2 -->
                        <td colspan="2" class="sub-col">
                            @if(isset($registros[1]) && $registros[1]->publicador)
                                {{ $registros[1]->publicador->nombre }} {{ $registros[1]->publicador->apellidos }}
                            @endif
                        </td>

                        <!-- Asignación 3 -->
                        <td colspan="2" class="sub-col">
                            @if(isset($registros[2]) && $registros[2]->publicador)
                                {{ $registros[2]->publicador->nombre }} {{ $registros[2]->publicador->apellidos }}
                            @endif
                        </td>

                        <!-- Asignación 4 -->
                        <td colspan="2" class="sub-col">
                            @if(isset($registros[3]) && $registros[3]->publicador)
                                {{ $registros[3]->publicador->nombre }} {{ $registros[3]->publicador->apellidos }}
                            @endif
                        </td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td class="sub-col">
                            @if(isset($registros[0]))
                                {{ $registros[0]->fecha_salida->format('d-m-y') }}
                            @endif
                        </td>
                        <td class="sub-col">
                            @if(isset($registros[0]) && $registros[0]->fecha_entrada)
                                {{ $registros[0]->fecha_entrada->format('d-m-y') }}
                            @endif
                        </td>
                        ...
                        <td class="sub-col">
                            @if(isset($registros[1]))
                                {{ $registros[1]->fecha_salida->format('d-m-y') }}
                            @endif
                        </td>
                        <td class="sub-col">
                            @if(isset($registros[1]) && $registros[1]->fecha_entrada)
                                {{ $registros[1]->fecha_entrada->format('d-m-y') }}
                            @endif
                        </td>

                        <!-- Asignación 3: Fechas -->
                        <td class="sub-col">
                            @if(isset($registros[2]))
                                {{ $registros[2]->fecha_salida->format('d-m-y') }}
                            @endif
                        </td>
                        <td class="sub-col">
                            @if(isset($registros[2]) && $registros[2]->fecha_entrada)
                                {{ $registros[2]->fecha_entrada->format('d-m-y') }}
                            @endif
                        </td>

                        <!-- Asignación 4: Fechas -->
                        <td class="sub-col">
                            @if(isset($registros[3]))
                                {{ $registros[3]->fecha_salida->format('d-m-y') }}
                            @endif
                        </td>
                        <td class="sub-col">
                            @if(isset($registros[3]) && $registros[3]->fecha_entrada)
                                {{ $registros[3]->fecha_entrada->format('d-m-y') }}
                            @endif
                        </td>
                    </tr>
                @endforeach
