...
<body>
    ...
    try {
        ...
        // Ver registro activo actual para verificar días asignado
        $registroActivo = $territorio->registros()
            ->whereNull('fecha_entrada')
            ->latest('fecha_salida')
            ->first();
            
        if ($registroActivo) {
            $diasAsignado = Carbon::parse($registroActivo->fecha_salida)->diffInDays(now());
            echo '<h2>📋 Registro activo actual:</h2>';
            echo '<div class="result">👤 Publicador: ' . $registroActivo->publicador->nombre_completo . '</div>';
            echo '<div class="result">📅 Fecha asignación: ' . $registroActivo->fecha_salida . '</div>';
            echo '<div class="result">⏰ Días asignado: ' . round($diasAsignado, 2) . '</div>';
            echo '<div class="result">🔢 Límite: 120 días</div>';
            echo '<div class="result">📊 Estado: ' . ($diasAsignado > 120 ? 'ATRASADO' : 'ACTIVO') . '</div>';
        }
